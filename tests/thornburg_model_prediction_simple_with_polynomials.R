...
#' calculate polynomials for a variable in a data.frame
calc_habitat_polynomials <- function(
  imbcr_df=NULL,
  allHabitatCovs=NULL,
  var=NULL,
  order=2
  ){
  if(inherits(imbcr_df, 'unmarked')){
    temp_df <- imbcr_df
    imbcr_df <- imbcr_df@siteCovs
  }
  # calculate an N order polynomial
  polynomials <- as.data.frame(matrix(
    poly(imbcr_df[ , var], degree=order), ncol=2))
    imbcr_df <- imbcr_df[ , !grepl(colnames(imbcr_df), pattern=var) ]
  names <- paste(c("first_","second_"), var, sep="")
  colnames(polynomials) <- names
    imbcr_df <- cbind(imbcr_df, polynomials)
  # optionally return a vector of our habitat variables if the user asked
  if(!is.null(allHabitatCovs)){
    allHabitatCovs <- allHabitatCovs[!grepl(allHabitatCovs, pattern=var)]
      allHabitatCovs <- c(allHabitatCovs, names)
    if(exists("df")){
      temp_df@siteCovs <- imbcr_df
      return(list(temp_df,allHabitatCovs))
    }
  } else {
    if(exists("temp_df")){
      temp_df@siteCovs <- imbcr_df
      return(list(df,allHabitatCovs))
    } else {
      return(imbcr_df)
    }
  }
...
units@data <- calc_habitat_polynomials(units@data, var="grass_ar")
units@data <- calc_habitat_polynomials(units@data, var="shrub_ar")
units@data <- calc_habitat_polynomials(units@data, var="wetland_ar")
units@data <- calc_habitat_polynomials(units@data, var="pat_ct")
